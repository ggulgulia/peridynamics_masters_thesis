...
def compare_PD_horizons_with_FE(mesh_lst, u_fe_conv, fig_cnt, data_path=None, material='steel', plot_=False, force=-5e8, vol_corr=True, struct_grd=False):
    ...
    for curr_mesh in mesh_lst[0:2]:
        ...
        horizons = np.array([0.11111667555600001, 0.166672235556])
        ...
        plt.figure()
        plt.plot(cell_cent_top[:,0], u_top_fe[:,1], color=colors[0], linewidth=2, label='FE Solution')
        ...
        plt.subplot(1,2,1)
        plt.plot(horizons, error, linewidth=2, marker='o', color='k', markersize=10)
        ...
        plt.xlabel('$\delta$ [m]', fontsize=20)
        plt.ylabel('abs difference in displacement', fontsize=18)
        plt.xticks(fontsize=14); plt.yticks(fontsize=14)
        plt.title('abs error b/w PD and FE vs Horizon size', fontsize=20)

        plt.subplot(1,2,2)
        plt.plot(horizons, rel_error, linewidth=2, marker='o', color='k', markersize=10)
        plt.gca().yaxis.set_major_formatter(mtick.FormatStrFormatter('%.1E'))
        plt.ylim(0.0, 20)
        plt.xlim(horizons[0]-horizons[0]/5, horizons[-1] + horizons[-1]/5)
        plt.xlabel('$\delta$ [m]', fontsize= 20)
        plt.ylabel('rel difference in displacement [%]', fontsize=18)
        plt.xticks(fontsize=14); plt.yticks(fontsize=14)
        plt.title('rel error(%) b/w PD and FE vs Horizon size', fontsize=20)
