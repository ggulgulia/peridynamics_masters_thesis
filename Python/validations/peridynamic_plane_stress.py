...
def solve_peridynamic_bar(horizon, m=mesh, nbr_lst=None, nbr_beta_lst=None, material='steel', omega_fun=None, plot_=False, force=-5e8, vol_corr=True, struct_grd=False):
    ...
    node_ids_dir = get_boundary_layers(cell_cent, el, num_lyrs, bc_loc, struct_grd) #dirichlet bc nodes
    node_ids_frc = get_boundary_layers(cell_cent, el, 2*num_lyrs, bc_loc, struct_grd)#force bc nodes
    ...
    #damage params : dummy numbers since we are not interested in
    # fracture for our tests : see test_fracture.py for actual test
    dmg_flag=False
    G0 = 0.0
    s0 = 1e10 
